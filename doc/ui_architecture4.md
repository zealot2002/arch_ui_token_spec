...
> 本文是颜色架构系列的第四篇，建议先阅读 [第一篇：三层颜色架构](/ui_architecture1)、[第二篇：7大色系×6种职能](/ui_architecture2)、[第三篇：Drawable 层设计](/ui_architecture3) 了解核心设计理念。
...
```
阶段1：定义 tv_base 和常用色系样式
阶段2：定义常用字号组合（tv_black_1_size_12/14/15/16）
阶段3：逐步替换现有布局中的重复属性
阶段4：新增 TextView 时直接使用 Style
```
